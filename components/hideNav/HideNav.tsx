...
export default function HideNav({
    ...
    const [navProperlyHidden, navProperlyHiddenSet] = useState(false)
    ...
    useEffect(() => {
        ...
        navProperlyHiddenSet(true)

        return () => {
            nav.classList.remove("clear")
            footer.classList.remove("clear")
            music?.classList.remove("clear")
        }
    }, [])

    return (
        <div className="blackFadeIn" style={{ "--animationPlaystate": navProperlyHidden ? "running" : "paused" } as React.CSSProperties}>
            {children}
